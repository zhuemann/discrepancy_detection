...
def discrepancy_datasetup_second_set(config):
    ...
    dataframe_location = os.path.join(dir_base,'Zach_Analysis/discrepancy_data/used_to_train_third_model/AI_Discrep_first_set_training_updated Apr 2023_new scoring.xlsx')
    ...
    for _, row in df.iterrows():
        ...
        if row["Report Type"] == "Preliminary":
            if not pd.isna(row["Discrepancy"]):
                ...
                print(df['Accession Number'] == row["Accession Number"])
