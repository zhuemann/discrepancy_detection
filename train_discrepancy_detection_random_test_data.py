...
def train_discrepancy_detection(config):
    ...
    #print(df)
    #dir_base = config["dir_base"]
    #dataframe_location = os.path.join(dir_base, 'Zach_Analysis/discrepancy_data/second_set_hand_cleaned_4_5_df.xlsx')
    ...
    if load_df_from_preset_location:
        ...
        train_loc = os.path.join(dir_base, 'Zach_Analysis/result_logs/discrepancy_detection/second_labeling_batch/saving_original_dataframe/seed' +str(config["seed"]) + '/train_df_seed' +str(config["seed"]) + '.xlsx')
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=3100, eta_min=1e-7, last_epoch=-1, verbose=False) #used 2100 for run 48
