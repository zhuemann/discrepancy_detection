...
def setup_dataloader(df, config, tokenizer, wordDict=None):
    ...
    if load_df_from_preset_location:
        train_loc = os.path.join(dir_base, 'Zach_Analysis/result_logs/discrepancy_detection/third_labeling_batch/data_folder_updated/seed' +str(config["seed"]) + '/train_df_seed' +str(config["seed"]) + '.xlsx')
        train_df = pd.read_excel(train_loc, engine='openpyxl')
        valid_loc = os.path.join(dir_base,'Zach_Analysis/result_logs/discrepancy_detection/third_labeling_batch/data_folder_updated/seed' +str(config["seed"]) + '/valid_df_seed' +str(config["seed"]) + '.xlsx')
